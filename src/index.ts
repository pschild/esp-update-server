...
app.get('/ota', (req: Request, res: Response) => {
    console.log(JSON.stringify(req.headers));
    ...
    */
    ...
    const currentVersion = headers['x-esp8266-version'];
    console.log(`${new Date().toISOString()}: ESP Chip ${chipId} is using version ${currentVersion}`);

    const filePath = path.resolve(__dirname, '..', 'binfiles', chipId, 'firmware.bin');
    res.sendFile(filePath, err => {
        if (err) {
            console.log(`No firmware file found.`);
            res.status(304).end();
        }
    });
});
